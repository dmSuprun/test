...
def get_student_average_in_course(course_slug, student_email):
    ...
    for test in assign_tests:
        try:
            testing_result_obj = UserWhoСompletedTest.objects.get(
                test=test, course=this_course, user=student)
        except Exception:
            continue
...
def average_mark_in_course(course_slug):
    ...
    try:
        this_course = get_course_by_slug(course_slug)
        completed_tests = UserWhoСompletedTest.objects.filter(
            course=this_course)
    except Exception:
        return ':('
...
def student_position_in_course(course_slug, student_email):
    ...
    for student in all_students:
        tmp_average = get_student_average_in_course(course_slug,
                                                    student.e_mail)
...
def get_average_mark_on_this_test_on_chossen_course(course_slug, test_slug):
    ...
    try:
        ...
        this_test_results = UserWhoСompletedTest.objects.filter(
            course=this_course, test=this_test)

        for result in this_test_results:
            if not check_teacher(result.user.email):
