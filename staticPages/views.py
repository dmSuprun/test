...
def index(request):
    ...
    if TYPE_PAGE == 'anonymous':
        context = {'type': TYPE_PAGE}
    elif TYPE_PAGE == 'teacher':
        # get teachers courses
        this_teacher_in_teachers_model = TeacherInCourse.objects.filter(
        ...
        for course in courses:
            students_in_this_course = StudentInCourse.objects.filter(
                course=course)
            LEN_ASSIGNED_TESTS = len(
                AssignedTest.objects.filter(course=course))
            num_of_students_on_course[course.name] = len(
                students_in_this_course)
            course_and_average_mark[course] = average_mark_in_course(
                course.course_slug)
            num_student_in_course_in_tamplate[course] = num_student_in_course(
                course.course_slug)
            ...
            for student in students_in_this_course:
                try:
                    ...
                    if len(
                            UserWhoСompletedTest.objects.filter(user=user_obj,
                                                                course=course)
                    ) == LEN_ASSIGNED_TESTS and LEN_ASSIGNED_TESTS != 0:
            ...
            if all_complete_students_num == 0:
                ...
                num_students_who_complete_all_assign_test[
                    course] = all_complete_students_num

            for test_tmp in assign_tests:

                average = get_average_mark_on_this_test_on_chossen_course(
                    course.course_slug, test_tmp.test.slug)
                test_and_it_average_mark_in_course[test_tmp.test] = average
                test_and_num_testcases[test_tmp.test] = len(
                    DataForTestingCode.objects.filter(
                        section__test=test_tmp.test))
                test_and_num_passage[test_tmp.test] = len([
                    i for i in UserWhoСompletedTest.objects.filter(
                        course=course, test=test_tmp.test)
                    if not (check_teacher(i.user.email))
                ])
        ...
        context = {
            ...
            'courses':
            courses_name,
            'num_students_on_course':
            num_of_students_on_course.items(),
            'title':
            f'{request.user.first_name} {request.user.last_name}',
            'averages_mark':
            course_and_average_mark.items(),
            'num_student_in_course':
            num_student_in_course_in_tamplate.items(),
            'count_tests':
            count_all_test_in_course.items(),
            'test_average_in_course':
            test_average_in_course.items(),
            'test_and_num_testcases':
            test_and_num_testcases.items(),
            'passage_num':
            num_passage_test_on_course.items(),
            'num_students_who_complete_all_assign_test':
            num_students_who_complete_all_assign_test.items()
        }
        ...
        for course in this_student_courses:
            all_course_teachers[course] = TeacherInCourse.objects.filter(
                course__name=course)
        ...
        for course, teachers in all_course_teachers.items():
            ...
            for teacher in teachers:
                try:
                    this_teacher_in_user_model = User.objects.get(
        ...
        for course in this_student_courses:
            ...
            course_and_num_students[course] = num_student_in_course(
                course.course_slug)
            ratings[course] = student_position_in_course(
                course.course_slug, request.user.email)
            corse_average[course] = average_mark_in_course(course.course_slug)
            my_average[course] = get_student_average_in_course(
                course.course_slug, request.user.email)
            my_results[course] = UserWhoСompletedTest.objects.filter(
        ...
        for course, testing_results in my_results.items():
            for test_result in testing_results:
                try:
                    cases_total_num[test_result.test] = len(
                        DataForTestingCode.objects.filter(
                            section__test=test_result.test)
                    )  # якщо додавати до перевірок і поле курс, то тут потрібно додати фильтр по курсаам
                except KeyError:
                    continue
        ...
        for course, testing_results in my_results.items():
            ...
            for testing_res in testing_results:
                tmp_tests_info[testing_res.test] = len(
                    CheckinResult.objects.filter(testing_result=True,
                                                 test=testing_res.test,
                                                 course=testing_res.course,
                                                 user=testing_res.user))
...
def about_page(request):
    ...
    context = {'type': get_header_type}
    return render(request,
                  'staticPages/static-pages/about.html',
                  context=context)
...
def tips_page(request):
    ...
    return render(request,
                  'staticPages/static-pages/tips.html',
                  context=context)
...
def faq_page(request):
    ...
    return render(request,
                  'staticPages/static-pages/faq.html',
                  context=context)
