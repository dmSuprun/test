...
def check_logic(answer,data_for_check, func_name):
    ...
    globals_and_locals_for_exec_func = {'open':None, 'print':None, 'settings':None}
    input_arguments = {}
    ...
    for one_data_obj in data_for_check:
        ...
        for one_type_val in arguments_type:
            ...
            if one_type_val == 'str' or one_type_val == 'int' or one_type_val == 'float' or one_type_val == 'bool' :
                ...
                elements_type_in_list=one_type_val[one_type_val.index('(')+1:one_type_val.index(')')]
                input_data=str(input_args[arguments_type.index(one_type_val)])+';'
                ...
                for item in stack_with_items:
                    if len(stack_with_items)-1 == stack_with_items.index(item):
                ...
                dict_types=one_type_val[one_type_val.index('(')+1:one_type_val.index(')')]
                TYPE_FOR_KEY=dict_types[:dict_types.index(':')]+'('
                TYPE_FOR_VAL = dict_types[dict_types.index(':')+1:len(dict_types)]+'('

                data_values=str(input_args[arguments_type.index(one_type_val)])
                stack_with_key=[]
                stack_with_val = []
                input_val = ''
                ...
                for one_el in  stack_with_key:
                    ...
                    dict_value_value = TYPE_FOR_VAL+stack_with_val[stack_with_key.index(one_el)]+')'
                    if stack_with_key.index(one_el) == (len(stack_with_key)-1):
        ...
        output_args_type_for_this_data_obj = output_arguments_types[one_data_obj]
        ...
        for one_type_val in out_arguments_type:
            ...
            if one_type_val == 'str' or one_type_val == 'int' or one_type_val == 'float' or one_type_val == 'bool':
                ...
                elements_type_in_list = one_type_val[one_type_val.index('(') + 1:one_type_val.index(')')]
                out_data = str(output_args[out_arguments_type.index(one_type_val)])+';'
                ...
                data_values = str(output_args[out_arguments_type.index(one_type_val)])
                stack_with_key = []
                stack_with_val = []
                out_val = ''
                for el in (data_values + ';'):
        ...
        try:
            ...
            RESULTS[one_data_obj] = True
        except Exception:
            ''' fail '''
            RESULTS[one_data_obj] = False
...
def check_result(request, course, test):
    ...
    all_user_answer_on_this_test = UserAnswer.objects.filter(user=request.user,test=this_test, course=this_course)
