...
def create_test(request):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            new_slug=transliteration(form.cleaned_data['name_test'])+'-vid-'+new_username
            create_test_data={
            ...
            if new_test_form.is_valid():
                ...
                form.add_error('name_test', ValidationError('Тест з такою назвою вже існує!'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if create_course.is_valid():
                create_course.save()
                get_course_obj = CourseConfig.objects.get(course_slug=new_data['course_slug'])
                if data_for_register['teacher_email'] == '':
                    data_for_register['e_mail']=request.user.email
                else:
                    data_for_register['e_mail']=data_for_register['teacher_email']
                ...
                if teacher_create_form.is_valid():
                    ...
                    if data_for_register['teacher_email'] == '':
                        return redirect(f'/course/detail/{new_data["course_slug"]}/')

                    else:

                        return redirect(f'/create/course/?status={True}&course={new_data["name"]}')
            else:
...
@login_required
def update_task(request, test_slug,pk_this_task, num_task_in_designer_template):
    if get_role(request) != 'teacher':
        return HttpResponse('<h1 style="text-align:center;position:relative; top:10%;">Доступ заборонено!</h1>', status=403)
    this_test = get_object_or_404(TestsConfig,slug=test_slug)
    this_task = get_object_or_404(TestsSection,pk=pk_this_task, test=this_test)
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('create_task', kwargs={'test_slug':this_test.slug}))
        ...
        data_for_testing_code_for_this_test = DataForTestingCode.objects.filter(section=this_task)
        all_testcases={}
        iteration=0
...
def delete_task(request, test_slug,pk_this_task):
    if get_role(request) != 'teacher':
        return HttpResponse('<h1 style="text-align:center;position:relative; top:10%;">Доступ заборонено!</h1>', status=403)
    get_test=get_object_or_404(TestsConfig,slug=test_slug)
    get_task = get_object_or_404(TestsSection,pk=pk_this_task, test=get_test).delete()
    ...
    if request.method =='POST':
        ...
        if form_who_create_task.is_valid():
            ...
            if model_form.is_valid():
                ...
                output_values = request.POST.getlist('out_value')
                prices = request.POST.getlist('price')
                this_test_section = get_object_or_404(TestsSection,test=data_for_model_form['test'], function_name=data_for_model_form['function_name'],question=data_for_model_form['question'] )
                ...
                return redirect(reverse('create_task',kwargs={'test_slug':get_this_test.slug}))
