...
class TestCreateForm(forms.Form):
    name_test = forms.CharField(
        label="Назва тесту",
        max_length=255,
        widget=forms.TextInput(attrs={
            'class':
            'form-control shadow-lg p-3 mb-5 bg-white mt-3 rounded ',
        }))
    theme_test = forms.CharField(
        label='Опис тесту',
        max_length=255,
        widget=forms.TextInput(attrs={
            'class':
            'form-control shadow-lg p-3 mb-5 mt-3 bg-white rounded ',
        }))
    max_test_result = forms.IntegerField(
    ...
    create_func_template = forms.BooleanField(
        label='Автоматичне створення скелету функції',
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}))
    autocomplete = forms.BooleanField(
        label='Дозвіл на автозавершення',
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}))
    pre_end_check = forms.BooleanField(
        label='Дозвіл на перевірку, перед завершеням тесту',
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}))
        ...
        for sym in function_name:
            if not (sym in ValidSymb):
                raise ValidationError(
                    f'Функція має не коректний({sym}) символ!')
            continue
        if function_name == 'recode_user_actual_result':
          raise ValidationError( f'Замініть назву функції!')
