...
def show_all_course(request):
    ...
    if user_role == 'anonymousStudent':
        ...
        courses = []
        teachers_on_course = {}
        this_teacher_info = TeacherInCourse.objects.filter(e_mail=request.user.email)
...
def show_detail_course(request, course_slug):
    ...
    for student in get_students:
        try:
            ...
            get_this_user_complete_test=UserWhoСompletedTest.objects.filter(user=tmp_user,course__course_slug=course_slug)
    ...
    context={
        ...
        'type': user_role,
        'post':post,
        'teachers':teachers_result.items(),
    ...
    if request.method == 'POST':
        ...
        if get_users_form.is_valid():
            try:
                ...
                get_users_form.add_error('csv_file_with_emails', 'Неможливо додати учнів до курсу!')
...
def method_name(course_slug, request, this_course, user_role):
    get_emails =  (i for i in open_csv_file(request.FILES['csv_file_with_emails'], user_role, course_slug))
...
def add_teachers(request,course_slug):
    ...
    if request.method == 'POST':
        ...
        if get_users_form.is_valid():
            try:
                add_teachers_logic(course_slug, request, this_course, user_role)
            except RuntimeError:

                get_users_form.add_error('csv_file_with_emails', 'Неможливо додати викладачів до курсу!')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if model_form.is_valid():
                ...
                form.add_error('email', ValidationError(model_form.errors.as_text()[12:len(model_form.errors.as_text())]))
    else:
        form = getEmail()
        ...
        if form.is_valid():
            ...
            if model_form.is_valid():
                ...
                form.add_error('email',
                               ValidationError(model_form.errors.as_text()[12:len(model_form.errors.as_text())]))
    else:
        form = getEmail()
...
def assign_test_on_course(request,course_slug):
    if get_role(request) != 'teacher':
        return HttpResponse('<h1 style="text-align:center;position:relative; top:10%;">Доступ заборонено!</h1>', status=403)
    this_course = get_object_or_404(CourseConfig,course_slug=course_slug)
    get_all_teachers_on_this_course = TeacherInCourse.objects.filter(course=this_course)
    all_teachers=[]
    test_queryset = []
    ...
    for teacher in all_teachers:
        ...
        test_for_this_iteration_teacher = TestsConfig.objects.filter(author_of_test=teacher)
...
def assign(request, course,test):
    ...
    if assign.is_valid():
        assign.save()
        return redirect(reverse('detail_course', kwargs={'course_slug':this_course.course_slug}))
    error = f'error={True}&test={this_test.name_test}&'
    return redirect(f'/course/{this_course.course_slug}/assign/?error={error}/')
...
def test_result(request, test, course):
    ...
    testing_code_result = CheckinResult.objects.filter(course=this_course, test=this_test)
    ...
    for student in students:
        try:
            ...
            users[f'{this_user.first_name} {this_user.last_name}'] = this_user.username
        except Exception:
            new_name = ''
            ind=0
...
def detail_test_result(request, test, course,username):
    ...
    this_course = CourseConfig.objects.get(course_slug=course)
    try:
        complete_result=UserWhoСompletedTest.objects.get(course=this_course, test=this_test, user__username=username)
    except Exception:
        complete_result=None


    this_user_answers = UserAnswer.objects.filter(test=this_test,course=this_course, user__username=username)
    ...
    try:
        checkin_results = CheckinResult.objects.filter(course=this_course, test=this_test, user__username=username)
...
def delete_assign_test(request, course_slug, test_slug):
    ...
    #checkin result
    linked_records_with_checkin_result = CheckinResult.objects.filter(course=this_course, test=this_test)
    linked_records_with_checkin_result.delete()

    linked_records = 0
    # completed test
    linked_records_with_complete_test = UserWhoСompletedTest.objects.filter(course=this_course, test=this_test)
    linked_records_with_complete_test.delete()

    #answer
    linked_records_with_answers = UserAnswer.objects.filter(course=this_course, test=this_test)
    linked_records_with_answers.delete()

    #assigned test
    linked_records_with_assigned_test= get_object_or_404(AssignedTest,course=this_course, test=this_test)
...
def delete_test_result(request, test_slug, course_slug, username):
    if get_role(request) != 'teacher':
        return HttpResponse('<h1 style="text-align:center;position:relative; top:10%;">Доступ заборонено!</h1>', status=403)
    # complete result
    complete_result = UserWhoСompletedTest.objects.filter( course__course_slug=course_slug, user__username=username, test__slug=test_slug)
    complete_result.delete()

    #answers
    answers = UserAnswer.objects.filter(course__course_slug=course_slug, user__username=username,test__slug=test_slug)
    answers.delete()

    #checkin

    checkin_result_obj = CheckinResult.objects.filter( course__course_slug=course_slug, user__username=username,test__slug=test_slug)
...
def update_course_name(request, course_slug):
    ...
    if request.method == 'POST':
        ...
        if request.POST['newName'] == '':
            return HttpResponse('<h1 style="text-align:center;position:relative; top:10%;">Неможливо оновити назву</h1>',
                            status=400)
        try:
            ...
            get_course.course_slug = transliteration(request.POST['newName'])+postfix
            get_course.save()
        except ValueError:
            get_course.name = request.POST['newName']
        ...
        return redirect(reverse('detail_course', kwargs={'course_slug':get_course.course_slug}))
    else:
